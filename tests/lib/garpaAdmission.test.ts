...
function passingOutcome(packet: ClaimPacket): MissionOutcome {
  ...
  const value = (text: string, evidenceCellIds: string[] = claimEvidence) => ({
    value: text,
    basis: "externally_supported" as const,
    evidenceCellIds,
    limitations: [],
  });
  return {
    ...
    operatingEnvironment: value("Controlled outdoor site", ["goal-environment"]),
    timeAndCoverageRequirement: value("Continuous coverage during a one-hour test window", ["goal-requirement"]),
    successMetrics: [
      {
        id: "m1",
